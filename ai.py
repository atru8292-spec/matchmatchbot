...
async def generate_reply(lead: dict, history: list[dict], user_text: str) -> dict:
    ...
    #     №51 напрямую (цена+детали+ссылка), даже холодному некалифицированному лиду.
    #     Владелец подтвердил 2026-08-22 и переподтвердил 2026-08-26: цену ивента
    #     (6,000 MXN) можно называть сразу, без квалификации — это НЕ то же самое, что
    #     цена основного сервиса ($10k), которую холодному не даём.
    #     ВАЖНО (регресс найден 2026-08-31): раньше это работало через AI-свободу у №2
    #     (ai_allowed=true) — модель сама решала дать цену вопреки шаблону-крючку. После
    #     того как №2 сделали дословным (ai_allowed=false, для стабильности формулировок),
    #     форс на №2 стал ЖЁСТКИМ гейтом и глушил цену ивента полностью. Поэтому теперь
    #     разделяем: вопрос про ивент → №51 напрямую; любой другой ценовой вопрос (про
    #     сервис) → №2 (крючок к квалификации, как и раньше).
    ...
    if lead.get("is_single") is not True:
        if _is_price_question(user_text) and not _explicitly_about_service(user_text) and (
            ...
            row = await db.get_scenario_row(51)
            if row:
                logger.info("холодный лид + вопрос про ЦЕНУ ИВЕНТА → форс №51 напрямую, был top=%s",
                            top.get("id") if top else None)
                row["score"] = 1.0
                scenarios, top = [row], row
        elif _is_price_question(user_text):
            row = await db.get_scenario_row(2)
            if row:
                logger.info("холодный лид + ценовой вопрос (не про ивент) → №2 (крючок), был top=%s",
                            top.get("id") if top else None)
                row["score"] = 1.0
                scenarios, top = [row], row
        ...
        # и так однозначно («квалифицирован + спрашивает про ивент → давай №51»), берём
        # №51 напрямую по id, не полагаясь на его RAG-score вообще (тот же паттерн, что у
        # холодного лида → №2).
        ...
        # получал в ответ цену и детали ивента. Теперь форсим №51 только если вопрос
        # реально про ивент (mentiona "evento" или interest уже 'event') — иначе (вопрос
        # про сервис) не форсим ничего, идёт обычная обработка/AI с полным контекстом.
        row = await db.get_scenario_row(51)
        if row:
            logger.info("тёплый лид + вопрос про ЦЕНУ ИВЕНТА → форс №51 (был top=%s)",
                        top.get("id"))
            row["score"] = 1.0
            scenarios, top = [row], row
