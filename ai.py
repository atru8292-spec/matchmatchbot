...
async def generate_reply(lead: dict, history: list[dict], user_text: str) -> dict:
    ...
    if lead.get("is_single") is not True:
        ...
        # анкету («soy soltero, 35») И спрашивает цену в ОДНОМ сообщении. RAG-эмбеддинг тогда
        # иногда матчит выше на общий сценарий подтверждения (напр. №4), а не на №51, и лид
        # уходит в свободную AI-генерацию, которая может пропустить [event_link] (регресс
        # найден 2026-08-21). Если №51 всё же есть среди кандидатов с достаточным score —
        # берём его вместо top, чтобы сработал детерминированный ответ с ссылкой.
        alt = next((s for s in scenarios if s.get("id") == 51 and s.get("score", 0) >= FIXED_SCORE), None)
        if alt:
