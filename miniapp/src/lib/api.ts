...
import type {
  EventSettings, LeadDetail, LeadsPage, LeadsQuery, Stats, TimelineItem, WhitelistClient,
} from "./types";
...
export async function sendMessage(phone: string, text: string): Promise<SendResult> {
  if (USE_MOCKS) {
  ...
  return apiSend<SendResult>(`/lead/${enc(phone)}/message`, "POST", { text });
}
